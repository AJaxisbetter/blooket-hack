    (async () => {
      ...
      var d = Object.values(document.querySelector('#app > div > div'))[1].children[1]['_owner'].stateNode,
  e = parseInt(prompt('How many blooks do you want?'))
e &&
  ((d.picking = false),
  (d.state.canGather = true),
  d.setState({ numBlooks: e + d.state.numBlooks }),
  d.props.firebase.setVal({
    id: d.props.client.hostId,
    path: 'a/'.concat(d.props.client.name, '/bs'),
    val: e,
  }),
  d.setState(
    {
      prize: 'gather',
      numBlooks: d.state.numBlooks,
      fadeOut: true,
    },
    function () {
      d.nextTimeout = setTimeout(function () {
        d.randomQ()
      }, 450)
    }   
  ))
...
function footer() {
    ...
    element.innerHTML = `<p>Made by gliz <br> My <a style="color: #0000ff;" href="https://twitter.com/therealglixzzy" target="_blank">twitter</a></p>`;
